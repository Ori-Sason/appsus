...
const BOOK_STORAGE_KEY = 'missBooksDB'
let gGoogleBooks = []
...
function getBooksMaxPrice() {
  return query()
    .then((books) => books.map((book) => book.listPrice.amount))
    .then((prices) => Math.ceil(Math.max(...prices) / 100) * 100)
}

function getBookById(id) {
  return query(null).then((books) => {
...
function getNextBook(bookId, diff) {
  return query(null).then(books => {
    let idx = books.findIndex(book => book.id === bookId)
    idx += diff

    if (idx === books.length) idx = 0
    else if (idx === -1) idx = books.length - 1
...
function addReview(bookId, review) {
  return query(null).then((books) => {
    ...
    if (!book.reviews || book.reviews.length === 0) book.reviews = [{ ...review, id: 0 }]
    else {
      const lastReviewId = book.reviews[book.reviews.length - 1].id
      book.reviews.push({ ...review, id: lastReviewId + 1 })
    }
...
function getGoogleBooks(searchKey) {
  const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchKey}`
  return axios.get(url)
    ...
    .then(items => items.map(item => ({ id: item.id, title: item.volumeInfo.title })))
}

function addGoogleBook(bookId) {
  const googleBook = gGoogleBooks.find(book => book.id === bookId)
  const { title, subtitle, authors, publishedDate, description, pageCount, categories, imageLinks, language } = googleBook.volumeInfo
  const book = {
    id: googleBook.id,
    title: title || '',
    subtitle: subtitle || '',
    authors: authors || ['Missing'],
    publishedDate: publishedDate && publishedDate.substring(0, 4) || 'Missing',
  ...
  return query(null).then(books => {
    const testBookIdx = books.findIndex(testBook => testBook.id === book.id)
    if (testBookIdx !== -1) return book
...
function _loadFromStorage() {
  return storageService.loadFromStorage(BOOK_STORAGE_KEY)
}

function _saveToStorage(books) {
  storageService.saveToStorage(BOOK_STORAGE_KEY, books)
}
